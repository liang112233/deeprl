...
class Env:
    ...
    def observe(self):
        if self.repre == 'image':
            ...
            compact_repr = np.zeros(self.pa.time_horizon * (self.pa.num_res + 1) +  # current work
                                    ...
                                    1,                                              # backlog indicator
                                    dtype=theano.config.floatX)  # this is just one way to configure the datatype as some kind of float such as float 16, float 32 etc.
    ...
    def get_reward(self):
        ...
        reward = 0
        for j in self.machine.running_job:
            reward += np.sum(float(np.sum(j.res_vec))*alpha)    # +np.sum(float(np.sum(j.res_vec))*alpha) is the resource utilization term ,alpha is the coefficient      self.pa.delay_penalty / float(j.len)
        ...
        for j in self.job_slot.slot:
            if j is not None:
                reward += 0     ##self.pa.hold_penalty / float(j.len) +

        for j in self.job_backlog.backlog:
            if j is not None:
                reward += 0   ##self.pa.dismiss_penalty / float(j.len) +

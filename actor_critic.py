...
class Actor(object):
    ...
    def get_prob(self,s):
        s = s[np.newaxis, :]
        probs = self.sess.run(self.acts_prob, {self.s: s})  # get probabilities for all actions   this is an array
        # print("l1:", self.sess.run(self.l1, {self.s: s}))
        # print("action probality", probs)
        #print("probs[0, self.a]",probs[0, self.a])
        return probs
    def get_one_act_prob(self, state):
        #state = state[np.newaxis, :]
        # states = np.zeros((1, input_height, input_width), dtype="float32")
...
class Critic(object):
    ...
    def learn(self, s, r, s_):
        ...
        v_ = self.sess.run(tf.squeeze(self.vout[0]), feed_dict={self.s: s_})  # value @ next state
        v =  self.sess.run(tf.squeeze(self.vout[0]), feed_dict={self.s: s})  # value @ next state

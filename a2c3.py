...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        # acts_prob = tf.nn.softmax(sy_logits_na)
        # sy_sampled_ac = acts_prob[0]  # this [0] reduce the dimension
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        # sy_logprob_n = tf.log((tf.squeeze(probs))[sy_ac_na[0]])  # sy_ac_na[1]     probs[0]
        # # #print("sy_logits_na shape", self.sess.run(sy_logits_na,{policy_parameters:policy_parameters}))
        # #     # YOUR_HW2 CODE_HERE
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        path = {"observation": np.array(obs, dtype=np.float32),
                ...
                "info": info}
        print("ob_no shape[0] from  trj", np.array(obs).shape[0] )
        return path
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        """
        sum_of_path_lengths = ob_no.shape[0]
        print("ob_no.shape[0] or 1",ob_no.shape[0],ob_no.shape[1],ob_no.shape[2])
        ...
        for i in range(self.num_target_updates):
            ...
            for j in range(self.num_grad_steps_per_target_update):
                ...
                #criticLoss = self.sess.run(self.critic_loss, feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: target})
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        # across paths
        #print("paths[observation].shape[0]",paths["observation"].shape[0])
        ob_no = np.concatenate([path["observation"] for path in paths])

        print([path["observation"].shape for path in paths])
        print("ob_no shape[0] from itr",ob_no.shape[0])

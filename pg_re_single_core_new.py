...
def get_traj(agent, env, episode_max_length, render=False):
    ...
    for _ in xrange(episode_max_length):
        ...
        a = (csprob_n > np.random.rand()).argmax()    ### the real place to choose an action
...
def launch(pa, pg_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    V_next = pg_network_new.CriticLearn(pa)
    ...
    for iteration in xrange(pa.num_epochs):
        ...
        for ex in xrange(pa.num_ex):
            ...
            """
            TD error  TD_error= r + Gamma*V_next - V_now ,   V_now is estimated by critic network,
            V_next is also estimated by the same network with s_next as input 

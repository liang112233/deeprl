...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.nn.relu, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        inph=tf.expand_dims(inph, axis=3) ### axis could be 3

        conv1 = tf.layers.conv2d(inputs=inph, filters=16, kernel_size=[3, 3], strides=[1, 1], padding='SAME', activation=tf.nn.relu)
        pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)
        conv2 = tf.layers.conv2d(inputs=pool1, filters=32, kernel_size=[2, 2], strides=[1, 1], padding='SAME',
                                 activation=tf.nn.relu)
        pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], strides=2)
        inph = tf.layers.Flatten()(pool2)
...
def plot_lr_curve(output_file_prefix, max_rew_lr_curve, mean_rew_lr_curve, slow_down_lr_curve,
    ...
    ax.plot(mean_rew_lr_curve, linewidth=2, label='A2C mean')
    ...
    plt.ylabel("Discounted Reward", fontsize=18)

    ax = fig.add_subplot(122)
    ax.set_color_cycle([cm(1. * i / num_colors) for i in range(num_colors)])

    ax.plot(slow_down_lr_curve, linewidth=2, label='A2C mean')
    ...
    plt.savefig(output_file_prefix + "pooling" + ".pdf")
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        with open(r"data/return_pool.txt", "ab") as f:
